...
3. **Forecast - `forecast_by_invoice`** (one row per unpaid invoice). This follows a
   ...
   - **Overdue handling - the headline caveat:** `already_overdue`
...
Sanity check the shape: the projected amounts must reconcile
...
(amplified by the overdue-backlog clamp); a thin tail into late 2026 comes from a
...
- The forecast logic mirrors the standalone step scripts: lag = `payment_date - due_date`, **median** at every level,
  member→tier→overall fallback with **N=3** and no tier count threshold, and the
  **projection-based** `already_overdue` clamp into week 1. `LAG_STAT` and
  `MIN_CUSTOMER_HISTORY` at the top of `pipeline.py` expose the lag knobs.
